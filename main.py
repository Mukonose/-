...
def analyze_with_groq(api_key, memo_list, period_label):
    ...
    # 【事前チェック】文字数が多すぎる場合はAPIを呼ばずに即エラー
    # ※日本語6000文字 ≒ 約8000~10000トークン (Groqの制限は6000TPM程度)
    if len(all_text) > 6000:
        return "⚠️ データ量が多すぎてTOKENが足りません。\n対象期間を短くするか（例：年単位ではなく月単位）、データを減らしてから再実行してください。"
    ...
    try:
        ...
        # APIのエラーメッセージにレート制限が含まれていたら、分かりやすく表示
        err_msg = str(e)
...
def create_pdf_report(report_text, period_label, caller_df, keyword_df):
    ...
    # エラーメッセージなどが返ってきている場合、そのままPDFにすると崩れるので整形
